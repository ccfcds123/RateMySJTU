...
function buildLocalCodeMap() {
  ...
  for (const el of allElements) {
    ...
    const match = text.match(/\(([A-Z]{2,5}[A-Z0-9]{3,8})\)\s*(.+)/);
    if (!match) continue;
...
function annotateSubRows(localCodeMap) {
  ...
  for (const row of candidateRows) {
    ...
    const classMatch = rowText.match(/([A-Z]{2,5}[A-Z0-9]{3,8})-\d{1,3}/);
...
function annotateTables() {
  for (const table of document.querySelectorAll("table")) {
    ...
    for (const row of table.querySelectorAll("tbody tr, tr")) {
      ...
      if (rowText.match(/([A-Z]{2,5}[A-Z0-9]{3,8})-\d{1,3}/) && rowText.match(/【(.+?)】/)) continue;
...
function isMainCourseRow(row) {
  ...
  if (/\([A-Z]{2,5}[A-Z0-9]{3,8}\)/.test(text)) return true;
  // Bare course code without sub-row markers (no CODE-NN, no 【】)
  if (/[A-Z]{2,5}[A-Z0-9]{3,8}/.test(text) &&
      !/[A-Z]{2,5}[A-Z0-9]{3,8}-\d{1,3}/.test(text) &&
